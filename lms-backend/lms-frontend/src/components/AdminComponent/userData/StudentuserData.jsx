...
const StudentuserData = () => {
  const navigate = useNavigate();
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const studentUsers = response.data.users.filter(users => users.role === "student");
          console.log("Response", studentUsers);
    ...
    const handleEdit = (id) => {
      navigate(`/admin/editstudentusers/${id}`);
    };
    ...
    return (
      <div className="container mx-auto">
        <h2 className="text-xl font-bold text-white text-center mt-10 bg-slate-600 mb-4 uppercase">Student User Table</h2>
        <table className="min-w-full table-auto  bg-blue-300 border border-gray-200">
          <thead>
            <tr>
          ...
          <tbody>
            {studentUsers.map((user, index) => (
              <tr key={user.id}>
                ...
                <td className="border border-gray-200 px-4 py-2">{user.admissionNo}</td>
