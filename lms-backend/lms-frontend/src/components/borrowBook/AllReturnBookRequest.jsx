    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:5000/api/admin/allReturnsBookRequest', {
                    headers: {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await Promise.all(selectedIds.map(id => 
                axios.delete(`http://localhost:5000/api/admin/acceptReturnsBookRequest/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${accessToken}`,
                        'Content-Type': 'application/json'
                    }
